...
childText;
    ...
    function groupAllRegex(reText, reOptions) {
        ...
        reText = preGroupallStrip(reText, reOptions).trim();
        ...
        if (sectionCur.parent != sectionRoot) {
            throw ('mismatched parenthesis');
        }

        // the first child and root get an end
        while (sectionCur) {
            if (sectionCur.endIndex == -1) sectionCur.close(reText.length);
